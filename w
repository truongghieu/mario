...
from scripts.player_topdown import TopdownPlayer
...
# Enemy
from scripts.particle import particle,running_effect, animate_object 
from scripts.particle_2 import animating_object
...
class Game:
    def __init__(self):
        ...
        self.data = {
            ...
            'player_dile' : Animation(load_images('player/idle_left'),5, True),
        ...
        self.test_animate_obj = animating_object(self.data['player_dile'],(DISPLAY_DIMENSIONS[0]//2,DISPLAY_DIMENSIONS[1]//2))
        self.m_tilemap = Tilemap(self)
        self.scroll = (0,0)
    def gameplay(self):
        while True:
            ...
            self.test_animate_obj.update()
            self.m_tilemap.update()
            #if self.particle.enabled == False:
            #    self.particle.enabled = True
            self.test_animate_obj.render(self.display)
            ...
            for event in events:
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_SPACE:
                        ...
                        self.test_animate_obj.enabled = not self.test_animate_obj.enabled
            self.clock.tick(60)
