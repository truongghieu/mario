...
class Editor:
    def __init__(self):
        ...
        a = 3.2
        b = 2
        self.screen = pygame.display.set_mode((640*a, 480*b))
        self.display = pygame.Surface((128*a, 96*b))
        ...
        self.map_file = "map/level_0.json"
    ...
    def gameplay(self):
        ...
        while self.game_state == game_state.gameplay:
            ...
            current_tile = self.assets[self.tile_list[self.tile_group]
            ...
            if self.ongrid:
                self.display.blit(current_tile, (tile_pos[0]*self.tilemap.tile_size -
                                  render_scroll[0], tile_pos[1]*self.tilemap.tile_size - render_scroll[1]))
            else:
            ...
            if self.right_clicking:
                if str(tile_pos[0])+';'+str(tile_pos[1]) in self.tilemap.tilemap:
                    del self.tilemap.tilemap[str(
                        tile_pos[0])+';'+str(tile_pos[1])]
                offgrid = self.tilemap.offgrid_tiles
                for tile in offgrid:
                    tile_image = self.assets[tile['type']][tile['variant']]
                    tile_rect = pygame.Rect(tile['pos'][0] - render_scroll[0], tile['pos']
                                            [1] - render_scroll[1], self.tilemap.tile_size, self.tilemap.tile_size)
                    ...
                    if event.button == 1:
                        ...
                        if not self.ongrid:
                            self.tilemap.offgrid_tiles.append({'type': self.tile_list[self.tile_group], 'variant': self.tile_variant, 'pos': (
                                mpos[0] + render_scroll[0], mpos[1] + render_scroll[1])})
