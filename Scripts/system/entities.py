...
class hammer(object):
    def __init__(self, image, position,defaut_pos = [220,190]):
        super().__init__(image, position)
        self.image = image.copy()
        self.enable = False
        self.spawn_time = image.img_duration * len(self.image.images)
        self.current_time = self.spawn_time
        self.defaut_pos = defaut_pos
        self.dame = 10
    def update(self):
        if self.enable:
            self.image.update()
    ...
    def copy(self):
        return hammer(self.image, self.position,self.enable)
...
class boss(object):
    ...
    def copy(self):
        return boss(self.image, self.position,self.enable)
